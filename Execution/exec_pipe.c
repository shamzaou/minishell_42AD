...
static void execute_left_pipe(t_pipecmd *pcmd, int *fd, t_params *params, int *exit_status, int *pid1, int *is_herdoc) {
    *pid1 = forking(params);
    if (*pid1 == 0)
...
static void execute_right_pipe(t_pipecmd *pcmd, int *fd, t_params *params, int *exit_status, int *pid1, int *is_herdoc) {
    int pid2 = forking(params);
    if (pid2 == 0)
